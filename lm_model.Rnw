...
options(width = 60)
if(require("IMA")){
    print("IMA is loaded correctly")
} else {
    print("trying to install IMA")
    install.packages("IMA",repos=c("http://rforge.net"))
    if(require('IMA')){
        print('IMA is installed and loaded')
    } else {
        stop("could not install IMA")
    }
...
dim(hm450)
dim(pData)
#hm450 contains the full annotation information and matches the dimension of hm450
rm(annot)
...
##provide the summary statistics for the methy450 object
...
cat(".......\nSplit the annotation file to 11 
    ...
    col5 = rep(as.character(annot[, "UCSC_CpG_Islands_Name"]), 
        listlength)
##we rep the col0, 1, 2, 3 according to how many sites each CpG probe is linked to 
...
for (s in slotNames(x.methy450)){
    if(grepl('Ind', s, ignore.case = TRUE)){
        ...
        mval.reg[[s]] <- indexregionfunc(indexlist= obj,
...
<<regression, cache = TRUE, echo = FALSE, include = TRUE, eval = TRUE>>=
dim(mvalues)
...
lmbehav <- function(m, pdat, bhv.score) {
...
site.regr <- foreach (y = y_vars, .verbose = TRUE) %dopar% {lmbehav(mvalues, pData, y)}
length(site.regr)
names(site.regr)
attributes(site.regr)
@

<<pv_plots, cache = TRUE, eval = TRUE>>=
pvplot<-function(pvs,titl){
    ...
    lines(sort(-log10(q0.025allc))[thin],expctd[thin],lwd=3,col=3)
    lines(sort(-log10(q0.975allc))[thin],expctd[thin],lwd=3,col=3)
    ##lines(sort(-log10(q0.025ballc)),expctd,lwd=3,col=4)
    ##lines(sort(-log10(q0.975ballc)),expctd,lwd=3,col=4)
    return(NULL)
}
    for (i in 1:9){
        pvplot(site.regr[[i]], y_vars[i])
    }
@

<<fn_to_iterate_regression, dev = 'pdf', fig.width=4, fig.height=4, cache = TRUE, echo = TRUE, eval = TRUE>>=
...
lmbehav2 <- function(m, pdat, scores) {
    ...
    pv.reg <- vector(mode = "list", length = 11)
    names(pv.reg) <- regions
#regress against the methylation values at each region
#do this for each of the 11 region categories
    for (r in names(m)){
        n_probes <- dim(m[[r]])[1]
        pv.reg[[j]] <- vector(mode= "numeric", length = n_probes)
        for (i in 1:n_probes) {
            pdat$mval <- m[[r]][i,]
            lm.out <- lm(as.formula(fmla), data = pdat)
            pv.reg[[r]][i] <- coef(summary(lm.out))["pdat$mval", "Pr(>|t|)"]
        }
    }
    return(pv.reg)
}
...
<<iterate_regression, dev = 'pdf', fig.width=4, fig.height=4, cache = TRUE, echo = TRUE, eval = FALSE>>=
#region based regression
reg.rgr <- foreach (y = y_vars, .verbose = TRUE) %dopar% {lmbehav2(mval.reg, pData, y)}
@

<<plots_of_p.sumryregr, dev = 'pdf', fig.width=4, fig.height=4, cache = TRUE, echo = TRUE, eval = FALSE>>=
