...
<<pv_plots, cache = TRUE, include = FALSE, eval = TRUE>>=
...
<<plots, cache = TRUE, echo = TRUE, eval = TRUE, out.width='.5\\linewidth', fig.show = 'hold'>>=

for (r in regions){
    length(regr.region.pvals[[1]][[r]])
}
...
for (i in 1:12){
    for (r in regions){
    pvplot(regr.region.pvals[[i]][[r]], paste(y_vars[[i]], r))
    }
}
@
\end{document}